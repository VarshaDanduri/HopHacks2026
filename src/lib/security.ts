...
export class SecurityUtils {
   ...
   */
  static async hasRole(userId: string, role: 'neighbor' | 'patron' | 'moderator' | 'admin'): Promise<boolean> {
    try {
      const user = await prisma.user.findUnique({
        where: { id: userId },
        select: { roles: true },
      ...
      switch (resourceType) {
        case 'wish':
          const wish = await prisma.wish.findUnique({
            where: { id: resourceId },
            select: { neighborId: true, status: true },
          });

          if (!wish) return false;

          // Owner can read/write their own wishes
          if (wish.neighborId === userId) {
        ...
        case 'fulfillment':
          const fulfillment = await prisma.fulfillment.findUnique({
            where: { id: resourceId },
            select: { patronId: true, wish: { select: { neighborId: true } } },
          });
          ...
          // Neighbor can read fulfillments for their wishes
          if (fulfillment.wish.neighborId === userId && action === 'read') return true;
