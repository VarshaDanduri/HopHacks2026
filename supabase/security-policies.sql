...
-- Neighbors can read their own wishes (full access)
CREATE POLICY "neighbors_read_own_wishes" ON public.wishes FOR SELECT
TO authenticated
USING (neighbor_id = auth.uid());

-- Neighbors can create their own wishes
CREATE POLICY "neighbors_create_own_wishes" ON public.wishes FOR INSERT
TO authenticated
WITH CHECK (
  neighbor_id = auth.uid() AND
  EXISTS (
    SELECT 1 FROM users 
    WHERE id = auth.uid() 
    AND (roles->>'neighbor')::boolean = true
  )
);

-- Neighbors can update their own wishes (only if draft or rejected)
...
-- Neighbors can delete their own wishes (only if draft or rejected)
CREATE POLICY "neighbors_delete_own_wishes" ON public.wishes FOR DELETE
TO authenticated
USING (
  neighbor_id = auth.uid() AND
  status IN ('DRAFT', 'REJECTED')
...
USING (
  EXISTS (
    ...
    AND (
      wishes.neighbor_id = auth.uid() OR 
...
-- Neighbors can create documents for their own wishes
CREATE POLICY "neighbors_create_wish_documents" ON public.wish_documents FOR INSERT
TO authenticated
WITH CHECK (
  EXISTS (
    SELECT 1 FROM wishes 
    WHERE wishes.id = wish_documents.wish_id 
    AND wishes.neighbor_id = auth.uid()
...
-- Neighbors can read fulfillments for their own wishes
CREATE POLICY "neighbors_read_wish_fulfillments" ON public.fulfillments FOR SELECT
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM wishes 
...
WITH CHECK (
  ...
  EXISTS (
    ...
    AND wishes.neighbor_id != auth.uid() -- Cannot fulfill own wishes
  )
);
...
CREATE INDEX IF NOT EXISTS idx_wishes_neighbor_id ON wishes(neighbor_id);
...
BEGIN
  ...
  VALUES (
    p_clerk_user_id,
    '{"neighbor": false, "patron": false, "moderator": false, "admin": false}'::jsonb,
    p_display_name,
    p_city,
    true
  )
